...
package
{
   ...
   public class Boss1Rush extends Boss
   {
      ...
      private static const IMGBODY_WIDTH:int = 100;
      
      private static const IMGBODY_HEIGHT:int = 100;
      ...
      public function Boss1Rush(param1:int, param2:int)
      {
         ...
         width = Number(IMGBODY_WIDTH);
         height = Number(IMGBODY_HEIGHT);
         this._turboMultiplier = Number(0.6);
         this._originX = param1;
         this._originY = param2;
         this._radiusMultTarget = Number(1);
         this._radiusMultCur = Number(1);
         this._attackMode = 0;
      ...
      override public function update() : void
      {
         ...
         x = Number(this._originX + 9 * 16 * Math.cos(_loc3_) * _loc4_);
         y = Number(this._originY + 7 * 16 * Math.sin(_loc3_) * _loc4_);
         this._eyes.x = Number(x - Math.cos(_loc2_) * 2 + 0.5);
         this._eyes.y = Number(y - Math.sin(_loc2_) * 2.5);
         this.fire(_loc1_);
         this._handRadius = Number(40 + 90 * Math.sin(Math.sin(_loc1_ * 5 / 3)));
         if(this._handRadius < 50)
         {
            this._handRadius = Number(50);
         }
         this._handRadius *= this._radiusMultCur;
         this._radiusMultCur = Number(this._radiusMultCur * 0.9 + this._radiusMultTarget * 0.1);
         var _loc5_:int = 0;
      ...
      public function fire(param1:Number) : void
      {
         ...
         if(param1 > this._firingPatternTimeout)
         {
            ...
            this._radiusMultTarget = Number(0);
            if(param1 > this._shotTimeout)
            {
               this._shotTimeout = Number(param1 + this.SHOT_DELAY * this._turboMultiplier);
               ++this._shotNum;
               ...
               {
                  this._firingPattern = int((this._firingPattern + 1) % 4);
                  this._firingPatternTimeout = Number(param1 + PATTERN_DELAY);
                  this._radiusMultTarget = Number(1);
                  this._shotNum = 0;
      ...
      override public function hurt(param1:Number) : void
      {
         ...
         {
            this._shotMax += 27;
            this._turboMultiplier = Number(0.19);
            this._attackMode = 2;
            play("normal2");
         }
         ...
         {
            this._shotMax += 9;
            this._turboMultiplier = Number(0.28);
            this._attackMode = 1;
            play("normal1");
         }
         super.hurt(param1);
