...
package spark.layouts.supportClasses
{
   ...
   public class LayoutBase extends OnDemandEventDispatcher
   {
      ...
      mx_internal function getScrollPositionDeltaToElementHelper(param1:int, param2:Number = NaN, param3:Number = NaN, param4:Number = NaN, param5:Number = NaN) : Point
      {
         ...
         if(!(_loc6_ = this.getElementBounds(param1)))
         {
            return null;
         }
         var _loc7_:Rectangle;
         if(!(_loc7_ = this.getScrollRect()) || !this.target.clipAndEnableScrolling)
         {
            return null;
         }
         ...
         var _loc12_:Number = Math.abs(_loc8_) < Math.abs(_loc9_) ? Number(_loc8_) : Number(_loc9_);
         var _loc13_:Number = Math.abs(_loc10_) < Math.abs(_loc11_) ? Number(_loc10_) : Number(_loc11_);
         if(!isNaN(param2))
         {
